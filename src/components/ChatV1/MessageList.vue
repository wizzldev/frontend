<template>
  ...
  >
    <li v-for="(msg, inx) in messageGroup" :key="inx">
      <Message @like="(id: number) => $emit('like', id)" :theme="theme" :messages="msg" />
    </li>
  </ul>
</template>
  ...
  for (let i = 0; i < props.messages.length; i++) {
    ...
    if (['message', 'file:file', 'file:image'].includes(msg.type)) {
      if (
        data.length == 0 ||
        (data.length > 0 &&
          (data[data.length - 1].sender.id != msg.sender.id ||
            ['message', 'file:file', 'file:image'].includes(data[data.length - 1].type)))
      ) {
