    ...
    <div>
      ...
      <ul class="max-w-[18rem]">
        <template v-for="msg in messages.messages" :key="msg.id">
          <li v-if="msg.reply" class="w-max max-w-full" :class="{ 'ml-auto': sentByMe }">
            <p class="text-xs text-gray-700 px-1">{{ replyMessage(msg.reply) }}</p>
            <div
              class="px-4 py-1.5 pb-4 -mb-4 w-max mt-1 rounded-xl max-w-full rounded-t-3xl bg-secondary opacity-50"
              :class="{
          ...
          <li
            v-else
            class="text-3xl relative cursor-pointer px-4 py-1.5"
            :class="{
              'ml-auto text-right': sentByMe
            }"
          >
            <p>{{ msg.content }}</p>
          </li>
