      ...
      <form v-on:submit.prevent class="px-4 w-full">
        ...
        <div class="mt-2">
          ...
          <p class="text-sm my-1 text-gray-400">
            {{ $t('Note') }}: {{ $t('Selected roles will be assigned to all users when they join, and you will be the creator.') }}
            <a class="transition-colors text-purple-400 hover:text-purple-500 focus:text-purple-500" :href="`${supportHost}/q?roles`">{{ $t('Read more') }}</a>
          </p>
          <template v-for="role in roles" :key="role">
            <button
              class="block w-full py-1.5 px-2 bg-secondary-all my-2 rounded-xl"
              :class="{'!bg-purple-500 !hover:bg-purple-400 !focus:bg-purple-400': usedRoles.includes(role)}"
              @click="toggle(role)"
            >{{ role }}</button>
          </template>
        </div>
